 ...
 * its platform actually looks: Reel as a script block + a real call-sheet (§8.9),
 * Carousel as swipeable IG slides + caption (§9.7), Thread as stacked X cards
 * with char counts (§10.8). Copy-to-clipboard everywhere; a tone chip per format
...
export default function Results({ onNew, onChangeVoice, kit, brandVoice }) {
  ...
  return (
    <section className="flex flex-1 flex-col gap-5">
      ...
      >
        {tab === 'reel' && <ReelView reel={kit.reel} tone={effectiveTone} />}
        {tab === 'carousel' && (
          <CarouselView carousel={kit.carousel} tone={effectiveTone} />
        )}
        {tab === 'thread' && (
          <ThreadView thread={kit.thread} tone={effectiveTone} />
        )}
      </div>
